...
void DrawRRDis_One( string File1,  string File2, In3str1v * paraFA04, Para_18 * para_18)
{
	...
	while(!LIST.eof())
	{
		...
		if(MeanRRV1!="NA")
		{
			...
			if (AA_it==ValueV1.end())
			{
				...
				ValueV1.insert( map < string,pair <int,int>  >::value_type(MeanRRV1,dataS));
			}
			else
			{				
				(AA_it->second).first++;
	...
	OUTR<<""
		...
		"read.table(\""<<OutPlotTmp<<"\",header=T)->data;\n"
		"meanRR<-data[,1];\nNum<-data[,2];\nsele<-data[,3];\n"
		"df <- data.frame(meanRR,Num);\n"
		"p <- ggplot(df, aes(x = meanRR, y = Num, col = sele,fill=factor(sele) ) ) + geom_bar(stat = \"identity\") + theme_economist();\n"
