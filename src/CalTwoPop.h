...
void DrawRRDis_Two( string File1,  string File2, In3str1v * paraFA04, Para_18 * para_18 ,string IDA, string IDB)
{
	...
	while(!LIST.eof())
	{
		...
		istringstream isone (line,istringstream::in);
		isone>>chr>>Start>>End>>MeanRRV1>>SumESV1>>CountV1>>MeanRRV2;
		if(MeanRRV1!="NA" && MeanRRV2!="NA" )
		{
			MeanRRV1=MeanRRV1.substr(0,4);
			MeanRRV2=MeanRRV2.substr(0,4);
			AA_it=ValueV1.find(MeanRRV1);
			if (AA_it==ValueV1.end())
			{
				pair <int,int> dataS (1,1);
				map < string,pair <int,int> >  BBHash;
				BBHash[MeanRRV2]=dataS;
				ValueV1.insert(map <string , map < string,pair <int,int> > >::value_type(MeanRRV1,BBHash));
			}
			else
			{
				BB_it=(AA_it->second).find(MeanRRV2);				
				if(BB_it==(AA_it->second).end())
				{
					pair <int,int> dataS (1,1);
					(AA_it->second).insert(map < string,pair <int,int> >::value_type(MeanRRV2,dataS));
				}
				else
				{
					(BB_it->second).first++;
	...
	int MaxXX=0;
	OUTPLOT<<"MeanRRA\tMeanRRB\tNum\tType\n";
	...
	OUTR<<""
		...
		"read.table(\""<<OutPlotTmp<<"\",header=T)->data;\n"
		"MeanRRA<-data[,1];\nMeanRRB<-data[,2];\nNum<-data[,3];\nType<-data[,4];\n"
		"df <- data.frame(MeanRRA,MeanRRB);\n"
		"p <- ggplot(df, aes(MeanRRA, MeanRRB,colour=Num,shape=Type)) + geom_point(size=1.5) +theme(legend.direction=\"horizontal\",legend.position= \"bottom\",legend.title = element_text(face =\"bold\",size=7));\n"
